...
class TransformerCommunication:
    ...
    def communication_isp(self, lins_scale, sp_scale):
        ...
        qkv_latency = 2 * self.allgather(qkv_wp_volume, self.lins_scale) + self.reducescatter(
            qkv_wp_volume, self.lins_scale
        )
        wo_latency = 2 * self.allgather(wo_wp_volume, self.lins_scale) + self.reducescatter(
            wo_wp_volume, self.lins_scale
        )
        mlp_w1_latency = 2 * self.allgather(mlp_w1_volume, self.lins_scale) + self.reducescatter(
            mlp_w1_volume, self.lins_scale
        )
    ...
    def communication_msp(self, lins_scale, sp_scale):
        ...
        sp_forward = (
            self.allgather(qkv_sp_volume, self.sp_scale)
            + self.reducescatter(wo_sp_volume, self.sp_scale)
            + self.allgather(mlp_w1_sp_volume, self.sp_scale)
            + self.reducescatter(mlp_w2_sp_volume, self.sp_scale)
        )

        sp_backward = (
            self.reducescatter(qkv_sp_volume, self.sp_scale)
            + self.allgather(wo_sp_volume, self.sp_scale)
            + self.reducescatter(mlp_w1_sp_volume, self.sp_scale)
        ...
        qkv_wp_latency = 2 * self.allgather(qkv_wp_volume, self.lins_scale) + self.reducescatter(
            qkv_wp_volume, self.lins_scale
        )
        wo_wp_latency = 2 * self.allgather(wo_wp_volume, self.lins_scale) + self.reducescatter(
            wo_wp_volume, self.lins_scale
        )
        mlp_w1_wp_latency = 2 * self.allgather(mlp_w1_wp_volume, self.lins_scale) + self.reducescatter(
            mlp_w1_wp_volume, self.lins_scale
        )
