...
class Initializer_Pipeline(ProcessGroupInitializer):
    ...
    def __init__(self, *args, **kwargs):
        ...
        assert self.world_size % self.pipeline_parallel_size == 0, f"world_size: {self.world_size} % pipeline_parallel_size: {self.pipeline_parallel_size}"
...
class Initializer_Tensor(ProcessGroupInitializer):
    ...
    def __init__(self, *args, **kwargs):
        ...
        assert self.world_size % self.tensor_parallel_size == 0, f"world_size: {self.world_size} % tensor_parallel_size: {self.tensor_parallel_size}"
...
class Initializer_Zero1(ProcessGroupInitializer):
    ...
    def __init__(self, *args, **kwargs):
        ...
        assert (
            self.world_size % (self.tensor_parallel_size * self.zero1_parallel_size) == 0
        ), f"world_size: {self.world_size} % tensor_parallel_size: {self.tensor_parallel_size} * zero1_parallel_size : {self.zero1_parallel_size} != 0 "
        assert (
            self.world_size % self.pipeline_parallel_size == 0
        ), f"world_size: {self.world_size} % pipeline_parallel_size: {self.pipeline_parallel_size}"
...
class Initializer_Zero1_ISP(ProcessGroupInitializer):
    ...
    def __init__(self, *args, **kwargs):
        ...
        assert (
            self.world_size % (self.pipeline_parallel_size * self.zero1_parallel_size) == 0
        ), f"world_size: {self.world_size} % pipeline_parallel_size: {self.pipeline_parallel_size} * zero1_parallel_size : {self.zero1_parallel_size} != 0 "
        assert (
            self.world_size % self.weight_zero1_size == 0
        ), f"world_size: {self.world_size} % weight_zero1_size: {self.weight_zero1_size} != 0 "
...
class Initializer_Weight(ProcessGroupInitializer):
    ...
    def __init__(self, *args, **kwargs):
        ...
        assert (
            self.world_size % self.weight_parallel_size == 0
        ), f"world_size: {self.world_size} % weight_parallel_size: {self.weight_parallel_size} != 0 "
...
class Initializer_Data(ProcessGroupInitializer):
    ...
    def __init__(self, *args, **kwargs):
        ...
        assert (
            self.world_size % self.data_parallel_size == 0
        ), f"world_size: {self.world_size} % data_parallel_size: {self.data_parallel_size} != 0 "
        assert (
            self.world_size % self.sequence_parallel_size == 0
        ), f"world_size: {self.world_size} % sequence_parallel_size: {self.sequence_parallel_size} != 0 "
        assert (
            self.world_size % self.pipeline_parallel_size == 0
        ), f"world_size: {self.world_size} % pipeline_parallel_size: {self.pipeline_parallel_size} != 0 "
...
class Initializer_Weight_Data(ProcessGroupInitializer):
    ...
    def __init__(self, *args, **kwargs):
        ...
        assert (
            self.world_size % self.pipeline_parallel_size == 0
        ), f"world_size: {self.world_size} % pp_size: {self.pipeline_parallel_size} != 0 "
        assert (
            self.world_size % (self.pipeline_parallel_size * self.weight_data_parallel_size) == 0
        ), f"world_size: {self.world_size} % pp_size {self.pipeline_parallel_size} * weight_data_parallel_size {self.weight_data_parallel_size} != 0 "
