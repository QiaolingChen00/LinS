...
class TransformerComputation:
    def __init__(
        self, b, s, h, num_layers, vocab_size, sp_scale, dtype_size, mlp_ratio, multiple_of, cost_data=None, ckpt=0
    ):
        self.b = b  # Batch size
    ...
    def _compute_attn(self):
        ...
        # (b, s, h/sp) * (b, s, h/sp)^T = (b, s, h/sp) * (h/sp, s, b) = (b, s, b)
        qkt_volume = self.dtype_size * self.b * self.s * self.s * self.h / self.sp_scale
        qkt_latency = self.get_linear_cost(qkt_volume)

        # Score dot V
        # (b, s, b) * (b, s, h/sp)
        score_v_volume = self.dtype_size * self.b * self.s * self.b * self.h / self.sp_scale
